...
McGonagall is a question flow framework. McGonagall uses [xState](https://xstate.js.org/docs/), a finite-state machine library, to help it navigate through the flow.
...
There's 4 key parts to creating a flow in McGonagall: `stateConfig`, `stateOptions`, `stateContext`, and `steps`. Depending on the project a scaffolder tool may be available to you by running `yarn generate mcg new`.

1. In the `.xstate.js` file, lay out the flow by adding state nodes to the `stateConfig` with the names for the conditions
2. In the `.steps.js` file, add an object to the steps array for each state node that you created in the `stateConfig`
3. In the `.js` file, fill out the `stateContext` object with any data you need to pull in
4. Starting with the first step, start adding the render card function for the step in `.step,js` and updating the `stateConfig` and the `stateOptions` with necessary actions and guards
